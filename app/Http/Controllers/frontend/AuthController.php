...
class AuthController extends Controller
{
    public function postSignin(Request $request)
    {
        ...
        $member_info = Member::where('member_email', 'LIKE', "%$request->member_email%" )
                        ->where('member_status', '1')
                        ->first();
                        // dd($member_info);
        if(isset($member_info)){
          // dd(Hash::check($request->member_password, $member_info->member_password));
            if(Hash::check($request->member_password, $member_info->member_password)) {
                ...
                if($member_info->online_status == '0'){
                    ...
                    if (Auth::guard('members')->attempt( [ 'member_email' => $request->member_email , 'password' => $request->member_password, 'member_status' => '1'] )) {
                        ...
                        $member_update = Member::where('_id', '=', Auth::guard('members')->user()->_id)->where('member_status', '1')->first();
