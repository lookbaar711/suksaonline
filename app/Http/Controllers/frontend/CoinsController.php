...
class CoinsController extends Controller
{
    ...
    public function refund(){
        ...
        //where('last_course_date_time', '>', $today)
        ...
        $i = 0;
        // dd($classrooms);
        foreach ($classrooms as $key_classroom => $value_classroom) {
            $cours = Course::where('_id', $value_classroom->course_id)->first();
            $now = date('Y-m-d H:i:s');
            $today = date('Y-m-d H:i:s',strtotime("-1 hour")); // ลบ 1 ชั่วโมง
            $max_refund_time = date($cours['last_course_date_time']); // วันเวลาสุดท้ายของ คอร์สที่เปิด
            // print_r('<pre>');
            // print_r($value_classroom->course_id);
            // print_r('<pre>');
            ...
            if(($now >= $max_refund_time) && ($today <= $max_refund_time) && ($cours['course_category'] != "school")){
              if (count($member_courses) > 0) {
                $check_ = collect($member_courses)->where("course_id",$value_classroom->course_id)->first();
                // print_r('<pre>');
                // print_r($check_['course_id']);
                // print_r('<pre>');
                // die;
                  if ($check_['course_id'] == "") {
                    $member_courses[] = $value_classroom;
                  }
