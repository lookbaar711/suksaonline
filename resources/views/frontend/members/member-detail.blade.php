      ...
      <div class="tab-content tabs" id="myTabContent">
        <div class="tab-pane fade show @if(!session('course')) active @endif" id="home" role="tabpanel" aria-labelledby="home-tab">
          ...
          <div class="col p-l-0" style="text-align: left">
                ...
                <br> <label for="inputAddress" style="font-weight: bold;"><i class="fa fa-school fa-lg" aria-hidden="true"></i> @lang('frontend/members/title.school_name_teacher') :</label><br>
                @php
                ...
                @if(!@empty($members->member_school))
                  @foreach ($members->member_school as $i => $item)
                    @if ($item)
                        @if ((isset(Auth::guard('members')->user()->member_lang) && (Auth::guard('members')->user()->member_lang=='en')) || (session('lang')=='en'))
                          <p class="showteacher" > @lang('frontend/members/title.school_name')  : {{ $item->school_name_en }}</p>
                        @else
                          <p class="showteacher" > @lang('frontend/members/title.school_name')  : {{ $item->school_name_th }}</p>
                        @endif
                    @else
                      @if ((isset(Auth::guard('members')->user()->member_lang) && (Auth::guard('members')->user()->member_lang=='en')) || (session('lang')=='en'))
                        <p class="showteacher" > @lang('frontend/members/title.school_name')  : -</p>
                      @else
                        <p class="showteacher" > @lang('frontend/members/title.school_name')  : -</p>
                      @endif
                    @endif
