      ...
      <div class="tab-content tabs" id="myTabContent">
        <div class="tab-pane fade dis_home show
          ...
          <form class="box1">
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                <label for="inputAddress" style="font-weight: bold;"><i class="fa fa-school fa-lg" aria-hidden="true"></i> @lang('frontend/members/title.school_name_teacher') :</label><br>

                @if(!@empty($members->member_school))
                  @foreach ($members->member_school as $i => $item)
                    @if ($item)

                        @if ((isset(Auth::guard('members')->user()->member_lang) && (Auth::guard('members')->user()->member_lang=='en')) || (session('lang')=='en'))
                          <div class="row">
                            <div class="col-md-12">
                                <label class="col-form-label">  @lang('frontend/members/title.school_name')  : {{ $item->school_name_en }}</label>
                            </div>
                          </div>
                        @else
                          <div class="row">
                            <div class="col-md-12">
                                <label class="col-form-label">  @lang('frontend/members/title.school_name')  : {{ $item->school_name_th }}</label>
                            </div>
                          </div>
                        @endif
                    ...
                    @else
                      @if ((isset(Auth::guard('members')->user()->member_lang) && (Auth::guard('members')->user()->member_lang=='en')) || (session('lang')=='en'))
                        <div class="row">
                          <div class="col-md-12">
                              <label class="col-form-label">  @lang('frontend/members/title.school_name')  : {{"-"}} </label>
                          </div>
                        </div>
                      @else
                        <div class="row">
...
<script src="/suksa/frontend/template/js/member_profile.js?v=01"></script>
<script src="/suksa/frontend/template/js/member_profile_edit.js?v=01"></script>
...
<script>
  ...
  function upfile(file){
    ...
    function del_course(course_id){
      ...
      var submit_button = '{{ trans('frontend/members/title.submit_button') }}';
      var cancel_button = '{{ trans('frontend/members/title.cancel_button') }}';
      var text_do_you_want_to_delete_this_course = '{{ trans('frontend/members/title.do_you_want_to_delete_this_course') }}';
      var text_this_course_has_been_deleted = '{{ trans('frontend/members/title.this_course_has_been_deleted') }}';
      var text_delete_course = '{{ trans('frontend/members/title.delete_course') }}';
      var this_course_cannot_be_deleted = '{{ trans('frontend/members/title.this_course_cannot_be_deleted') }}';
      var ok = '{{ trans('frontend/members/title.ok') }}';
      var course_cannot_be_deleted = '{{ trans('frontend/members/title.The_course_cannot_be_deleted') }}';
      // console.log(text_this_course_has_been_deleted);
      // console.log(text_delete_course);
      Swal.fire({
        title: text_do_you_want_to_delete_this_course,
        type: 'warning',
        ...
        confirmButtonText: submit_button
          ...
          if (result.value) {
            Swal.showLoading();
            $.ajax({
              url: window.location.origin + "/members/del_coures/"+course_id,
              method:'get',
              success:function(data) {
                Swal.showLoading();
                // console.log(data);
                if (data == "success") {
                  ...
                  if (data == "The course cannot be deleted") {
                    Swal.fire({
                      type: 'info',
                      title: course_cannot_be_deleted,
                      showCancelButton: false,
                      focusConfirm: false,
                      confirmButtonColor: '#28a745',
