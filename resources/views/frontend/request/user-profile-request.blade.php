                    ...
                    <div class="container">
                      <div class="row">
                        ...
                        <div class="col-sm-6">
                          ...
                          <p class="showteacher">
                            <img class="p-b-5 " style="width: 20px; " src="{{ asset ('suksa/frontend/template/images/icons/ico_03.png') }}">&nbsp;&nbsp;@lang('frontend/users/title.student_email') : {{$members->member_email}}
                          </p>
                          <p class="showteacher">
                            <img class="p-b-5 " style="width: 20px; " src="{{ asset ('suksa/frontend/template/images/icons/ico_04.png') }}">&nbsp;&nbsp;@lang('frontend/users/title.mobile_no') : {{$members->member_tell}}
                          </p>
                          <p class="showteacher">
                            @php
                            ...
                            @if(!@empty($members->member_school))
                              @foreach ($members->member_school as $i => $item)
                                @php
                                  // dd(collect($item->school_student)->where('student_email',$members->member_email));
                                  $student_classroom = collect($item->school_student)->where('student_email',$members->member_email);
                                  // dd($student_classroom);
                                @endphp
                                @if ($item)
                                  @foreach ($student_classroom as $key_classroom => $value_classroom)
                                    @if (!empty($value_classroom['student_classroom']))
                                      @if ((isset(Auth::guard('members')->user()->member_lang) && (Auth::guard('members')->user()->member_lang=='en')) || (session('lang')=='en'))
                                        <i class="fa fa-school fa-lg" aria-hidden="true"></i> @lang('frontend/members/title.school_name_teacher') :  {{$item->school_name_en." ".$value_classroom['student_classroom']}}
                                      @else
                                        <i class="fa fa-school fa-lg" aria-hidden="true"></i> @lang('frontend/members/title.school_name_teacher') :  {{$item->school_name_th." ".$value_classroom['student_classroom']}}
                                      @endif
                                    @else
                                      @if ((isset(Auth::guard('members')->user()->member_lang) && (Auth::guard('members')->user()->member_lang=='en')) || (session('lang')=='en'))
                                        <i class="fa fa-school fa-lg" aria-hidden="true"></i> @lang('frontend/members/title.school_name_teacher') :  {{$item->school_name_en}}
                                      @else
                                        <i class="fa fa-school fa-lg" aria-hidden="true"></i> @lang('frontend/members/title.school_name_teacher') :  {{$item->school_name_th}}
                                      @endif
                                    @endif
                                  @endforeach
                                ...
                                @else
                                  @if ((isset(Auth::guard('members')->user()->member_lang) && (Auth::guard('members')->user()->member_lang=='en')) || (session('lang')=='en'))
                                    <i class="fa fa-school fa-lg" aria-hidden="true"></i> @lang('frontend/members/title.school_name_teacher') :  {{"-"}}
                                  @else
                                    <i class="fa fa-school fa-lg" aria-hidden="true"></i> @lang('frontend/members/title.school_name_teacher') :  {{"-"}}
                                  @endif
                                @endif
