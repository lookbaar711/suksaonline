...
var profile_contact = {
  ...
  new_page_contact : (data) => {
    ...
    var text_del_button = (lang=='en')?'Delete course':'ลบคอร์สเรียน';
    ...
    $.each(data.cousres.data, function(key, value){
      ...
      if(value.course_category == 'Public'){
      ...
      else if(value.course_category == 'Private'){
        ...
        icon = '<i class="fs-16 fa fa-school fa-lg" style="font-size:16px;"> </i> <label calss="fs-16">'+view_student_list+'</label>';
        list_student = '<button type="button" class="btn btn-link fs-16" onclick="profile_contact.gen_modal_list_student(`'+value._id+'`)">'+student_list+'</button>';
      }
      ...
      var student = '';
      if(value.student > 0){
        if (value.course_category == "School") {
      ...
      if(value.course_category == 'Public'){
          ...
          $.each(value.course_date, function(index, el){
            ...
            if((current_datetime.getTime() >= course_start_datetime.getTime())
              ...
              if(value.classroom_status == 1){
                ...
                if (value.link_open) {
                  btn = `<a href="`+window.location.origin+`/classroom/check/`+value.link_open+`" class="btn  button-s" target="_blank" style="margin-top: 0px; margin-top: 0px; background: #6ab22a; color: white;" >`
                        ...
                        +data.course_detail+`</a>`; //00112
                } else {
                  btn = `<button type="button" class="btn  button-s" target="_blank" style="margin-top: 0px; margin-top: 0px; background: #6ab22a; color: white;" data-role="disabled">`
                        +join_button+` </button> <a href="`+window.location.origin+`/courses/`+value._id+`" class="btn btn-outline-dark  " style="border-radius: 20px; font-size: 14px">`
            ...
            /**** ถ้าวันเวลาปัจจุบัน ไม่ตรงกับ วันเวลาที่เปิดคอร์ส ****/
              ...
              first_open_course.setMinutes(first_open_course.getMinutes() - 8);

              first_open_course_add = new Date(value.course_date_start+" "+value.course_time_start);
              first_open_course_add.setMinutes(first_open_course_add.getMinutes() + 1);

              var last_open_course = new Date(value.last_course_date_time);
              last_open_course.setMinutes(last_open_course.getMinutes());
                ...
                //ถ้าจำนวนนักเรียนไม่เต็มโควตา
                  ...
                  if(value.classroom_status == 1){
                    ...
                    // if (value.student_status <= 0) {
                      btn_del = '<i class="fs-16 fa fa-trash"> <a href="#" class="btn-link" style="font-size:16px; color: red;font-weight: normal;" onclick="del_course(`'+value._id+'`)">'+text_del_button+'</a> </i> ';
                    // }
                    btn = `<span class="btn btn-button-open" >`+ data.openning_register +
                        `</span> <a href="`+window.location.origin+`/courses/`+value._id+
                        `" class="btn btn-outline-dark  " style="border-radius: 20px; font-size: 14px">`
              ...
              //ถ้าวันเวลาปัจจุบัน มากกว่า วันเวลาที่เปิดสอน (มีคนสมัครเข้ามา)
              else{
                if (value.student_status == 0 && current_datetime.getTime() > first_open_course_add.getTime() && value.course_category != "School") { // เช็คว่ามีคนมาสมัครเรียนคอร์สนี้หรือไม่ถ้าไม่มีคนสมัครให้ลบได้ course Private
                  //ให้แสดงปุ่ม รายละเอียด และปุ่มลบคอร์ส
                  btn_del = '<i class="fs-16 fa fa-trash"> <a href="#" class="btn-link" style="font-size:16px; color: red;font-weight: normal;" onclick="del_course(`'+value._id+'`)">'+text_del_button+'</a> </i> ';

                  btn = '<a href="'+window.location.origin+'/courses/'+value._id+'" class="btn btn-outline-dark  " style="border-radius: 20px; font-size: 14px">'
                        +data.course_detail+'</a>'; //0041
                }else {
                  if (current_datetime.getTime() > last_open_course.getTime()) {
                    btn_del = '<i class="fs-16 fa fa-trash"> <a href="#" class="btn-link" style="font-size:16px; color: red;font-weight: normal;" onclick="del_course(`'+value._id+'`)">'+text_del_button+'</a> </i> ';
                  }
        ...
        //ถ้ายังไม่เปิดรับสมัคร
          ...
          $.each(value.course_date, function(index, el){
            ...
            var course_start_datetime_add = new Date(value.course_date_start+" "+value.course_time_start);
            course_start_datetime.setMinutes(course_start_datetime.getMinutes() - 8);
            course_start_datetime_add.setMinutes(course_start_datetime_add.getMinutes() + 1);
            ...
            //ถ้าวันเวลาปัจจุบัน มากกว่าหรือเท่ากับ วันเวลาที่เปิดสอน
              ...
              if (value.student_status == 0 && current_datetime.getTime() > course_start_datetime_add.getTime() && value.course_category != "School") { // เช็คว่ามีคนมาสมัครเรียนคอร์สนี้หรือไม่ถ้าไม่มีคนสมัครให้ลบได้
                //ให้แสดงปุ่ม รายละเอียด และปุ่มลบคอร์ส
                ...
                btn = '<a href="'+window.location.origin+'/courses/'+value._id+'" class="btn btn-outline-dark  " style="border-radius: 20px; font-size: 14px">'
                      +data.course_detail+'</a>'; //0061 d1.getTime()
              }else {
                //ให้แสดงปุ่ม รายละเอียด
                btn = `<a href="`+window.location.origin+`/courses/`+value._id+
                      `" class="btn btn-outline-dark" style="border-radius: 20px; font-size: 14px">`
      ...
      //Private Course
          ...
          $.each(value.course_date, function(index, el){
            ...
            if(((current_datetime.getTime() >= course_start_datetime.getTime())
              ...
              if(value.course_student_limit == value.student){
                ...
                if (value.link_open) {
                  ...
                  btn = `<button type="button" class="btn  button-s" target="_blank" style="margin-top: 0px; margin-top: 0px; background: #6ab22a; color: white;" data-role="disabled">`
                        +join_button+` </button> <a href="`+window.location.origin+`/courses/`+value._id+`" class="btn btn-outline-dark  " style="border-radius: 20px; font-size: 14px">`
                        +data.course_detail+`</a> `; //0071111
                }
                return false;
              }
              else{
                if (value.student_status >= 50 && value.course_category == "School") {
                  if (value.link_open) {
                    btn = `<a href="`+window.location.origin+`/classroom/check/`+value.link_open+`" class="btn  button-s" target="_blank" style="margin-top: 0px; margin-top: 0px; background: #6ab22a; color: white;" >`
                          +join_button+` </a> <a href="`+window.location.origin+`/courses/`+value._id+`" class="btn btn-outline-dark  " style="border-radius: 20px; font-size: 14px">`
                          +data.course_detail+`</a>`; //007121
                  } else {
                    btn = `<button type="button" class="btn  button-s" target="_blank" style="margin-top: 0px; margin-top: 0px; background: #6ab22a; color: white;" data-role="disabled">`
                          +join_button+` </button> <a href="`+window.location.origin+`/courses/`+value._id+`" class="btn btn-outline-dark  " style="border-radius: 20px; font-size: 14px">`
                          +data.course_detail+`</a>`; //0071875
                  }
                  return false;
                }
                else {
                  if (value.student_status >= 50 && current_datetime.getTime() < course_end_datetime.getTime() && value.course_category != "School") { // เช็คว่ามีคนสมัครเกิน 50% ไม่ + เวลาปัจจุบันต้องน้อยกว่าเวลาปิดคอร์ส
                    if (value.link_open) {
                      btn = `<a href="`+window.location.origin+`/classroom/check/`+value.link_open+`" class="btn  button-s" target="_blank" style="margin-top: 0px; margin-top: 0px; background: #6ab22a; color: white;" >`
                            +join_button+` </a> <a href="`+window.location.origin+`/courses/`+value._id+`" class="btn btn-outline-dark  " style="border-radius: 20px; font-size: 14px">`
                            +data.course_detail+`</a>`; //0073789
                    } else {
                      btn = `<button type="button" class="btn  button-s" target="_blank" style="margin-top: 0px; margin-top: 0px; background: #6ab22a; color: white;" data-role="disabled">`
                            +join_button+` </button> <a href="`+window.location.origin+`/courses/`+value._id+`" class="btn btn-outline-dark  " style="border-radius: 20px; font-size: 14px">`
                            +data.course_detail+`</a> `; //007354689
                    }
                    return false;
                  }
                  else {
            ...
            /**** ถ้าวันเวลาปัจจุบัน ไม่ตรงกับ วันเวลาที่เปิดคอร์ส ****/
              ...
              if(current_datetime.getTime() < course_start_datetime.getTime()){
                ...
                if (value.course_category == "School" || value.course_category == "Private" && value.course_price <= 0 ) {
                  if ( current_datetime.getTime() < course_start_datetime.getTime()) {
                    if (current_datetime.getTime() > last_open_course.getTime()) {
                      btn_del = '<i class="fs-16 fa fa-trash"> <a href="#" class="btn-link" style="font-size:16px; color: red;font-weight: normal;" onclick="del_course(`'+value._id+'`)">'+text_del_button+'</a> </i> ';
                      // return false;
                ...
                if (value.course_category == "School" && current_datetime.getTime() < course_start_datetime.getTime() ) {
                  btn_del = '<i class="fs-16 fa fa-trash"> <a href="#" class="btn-link" style="font-size:16px; color: red;font-weight: normal;" onclick="del_course(`'+value._id+'`)">'+text_del_button+'</a> </i> ';
                }
              ...
              //ถ้าวันเวลาปัจจุบัน มากกว่า วันเวลาที่เปิดสอน
                ...
                if (value.student_status >= 50 && current_datetime.getTime() < course_end_datetime.getTime()) { // เช็คว่ามีคนสมัครเกิน 50% ไม่ + เวลาปัจจุบันต้องน้อยกว่าเวลาปิดคอร์ส
                  if (value.link_open) {
                    btn = `<a href="`+window.location.origin+`/classroom/check/`+value.link_open+`" class="btn  button-s" target="_blank" style="margin-top: 0px; margin-top: 0px; background: #6ab22a; color: white;" >`
                          +join_button+` </a> <a href="`+window.location.origin+`/courses/`+value._id+`" class="btn btn-outline-dark  " style="border-radius: 20px; font-size: 14px">`
                          +data.course_detail+`</a>`; //0091
                  } else {
                    btn = `<button type="button" class="btn  button-s" target="_blank" style="margin-top: 0px; margin-top: 0px; background: #6ab22a; color: white;" data-role="disabled">`
                          +join_button+` </button> <a href="`+window.location.origin+`/courses/`+value._id+`" class="btn btn-outline-dark  " style="border-radius: 20px; font-size: 14px">`
                          +data.course_detail+`</a>`; //0091
                  ...
                  return false;
                    ...
                    if (value.course_category == "School" || value.course_category == "Private" || value.student_status <= 50) {
                      // if ( current_datetime.getTime() < course_start_datetime.getTime()) {
                        if (current_datetime.getTime() > last_open_course.getTime()) {
                          btn_del = '<i class="fs-16 fa fa-trash"> <a href="#" class="btn-link" style="font-size:16px; color: red;font-weight: normal;" onclick="del_course(`'+value._id+'`)">'+text_del_button+'</a> </i>';
                          // return false;
        ...
        //ถ้าสถานะคอร์สเปลี่ยนเป็นยกเลิก course_status = cancel
          ...
          if (value.student_status == 0 && current_datetime.getTime() > course_start_datetime_add.getTime() && value.course_category != "School") { // เช็คว่ามีคนมาสมัครเรียนคอร์สนี้หรือไม่ถ้าไม่มีคนสมัครให้ลบได้ course Private
            //ให้แสดงปุ่ม รายละเอียด และปุ่มลบคอร์ส
            if (value.course_price <= 0) {
              if (current_datetime.getTime() > last_open_course.getTime() || value.course_status == "cancel") {
                btn_del = '<i class="fs-16 fa fa-trash"> <a href="#" class="btn-link" style="font-size:16px; color: red;font-weight: normal;" onclick="del_course(`'+value._id+'`)">'+text_del_button+'</a> </i>';
              }
            }
            btn = '<a href="'+window.location.origin+'/courses/'+value._id+'" class="btn btn-outline-dark  " style="border-radius: 20px; font-size: 14px">'
                  +data.course_detail+'</a> '; //000101
          }else {
            if (value.course_price <= 0 && current_datetime.getTime() < course_start_datetime_add.getTime() && value.student_status == 0) {
              btn_del = '<i class="fs-16 fa fa-trash"> <a href="#" class="btn-link" style="font-size:16px; color: red;font-weight: normal;" onclick="del_course(`'+value._id+'`)">'+text_del_button+'</a> </i> ';
            }
            if (value.course_status == "cancel") {
              btn_del = '<i class="fs-16 fa fa-trash"> <a href="#" class="btn-link" style="font-size:16px; color: red;font-weight: normal;" onclick="del_course(`'+value._id+'`)">'+text_del_button+'</a> </i> ';
            }
            //ให้แสดงปุ่ม รายละเอียด Private
            btn = `<a href="`+window.location.origin+`/courses/`+value._id+
                  `" class="btn btn-outline-dark" style="border-radius: 20px; font-size: 14px">`
              ...
              <div class="container">
                <div class="row">
                  ...
                  <div class="col-sm-5">
                    ...
                    <p class="fs-16 header-noti" color: black;">`+icon+` `+student+` `+list_student+`</p> `+course_file+`
                    <p class="fs-16 header-noti" color: black;">`+btn_del+`</p>
                  </div>
  ...
  modal_list_student :(data) => {
    ...
    if (data.length > 0) {
        var list = '';
        if ((data[0].category == 'Public') || (data[0].category == 'School')) {
          st_bt = 'Public';
          list = list = data;
        }
        else {
      ...
      $.each(data.alerts.data, function(key, value) {
        ...
        if (data.lang == "en") {
          if(value.noti_type == 'open_course_teacher'){
            ...
            <hr>`;
          } else if(value.noti_type == 'checkend_homework'){
            var room_name = '';
                ...
                alerts += '<div class="container">';
                    alerts += '<div class="row form-row">';
                        alerts += '<div class="col-sm-10">';
                            alerts += '<p class="fs-16 text-head-noti"><b>HomeworkChecked</b></p>';
                            alerts += '<p class="fs-16 header-noti">It time Check homework '+ room_name +' </p>';
                            alerts += '<p class="fs-16 header-noti">Of instructor '+ value.member_fullname +' </p>';
                        ...
                        alerts += '<div class="col-sm-2 text-right">';
                            alerts += '<a href="'+window.location.origin+'/homework/teacher/assignment/'+value.assignment_id+'" target="_blank">';
                                alerts += '<button class="btn btn-noti" style="border-radius: 20px; font-size: 14px; color: white; width: auto;" >Check Homework</button>';
                            alerts += '</a>';
                        alerts += '</div>';
                    alerts += '</div>';
                alerts += '</div>';
          ...
          if(value.noti_type == 'open_course_teacher'){
            ...
            alerts += '<div class="form-row">';
                alerts += '<div class="container">';
                    alerts += '<div class="row form-row">';
                        alerts += '<div class="col-sm-10">';
                            alerts += '<p class="fs-16 text-head-noti"><b>ตรวจการบ้าน</b></p>';
                            alerts += '<p class="fs-16 header-noti">ถึงเวลาตรวจการบ้าน'+ room_name +' </p>';
                            alerts += '<p class="fs-16 header-noti">ของผู้สอน '+ value.member_fullname +' </p>';
                            alerts += '<div class="row">';
                                alerts += '<div class="col-sm-7" >';
                                    alerts += '<p class="fs-16 header-noti"><i style="font-size:16px" class="fa">&#xf073;</i> '+moment(value.created_at).format('DD/MM/YYYY HH:mm') +'</p>';
                                alerts += '</div>';
                        ...
                        alerts += '<div class="col-sm-2 text-right">';
                            alerts += '<a href="'+window.location.origin+'/homework/teacher/assignment/'+value.assignment_id+'" target="_blank">';
                                alerts += '<button class="btn btn-noti" style="border-radius: 20px; font-size: 14px; color: white; width: auto;" >ตรวจการบ้าน</button>';
                            alerts += '</a>';
                        alerts += '</div>';
                    alerts += '</div>';
                alerts += '</div>';
