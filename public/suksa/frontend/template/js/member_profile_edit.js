$(document).ready(function(){
    ...
    $('#member_education1').click(function(){
      if ($('#member_education1').is(':checked') == true) {
        $( "#member_institution" ).prop( "disabled", false );
      }else {
        $( "#member_institution" ).prop( "disabled", true );
      }
     });

    $('#member_education2').click(function(){
      if ($('#member_education2').is(':checked') == true) {
        $( "#member_major2" ).prop( "disabled", false );
        $( "#member_institution2" ).prop( "disabled", false );
      }else {
        $( "#member_major2" ).prop( "disabled", true );
        $( "#member_institution2" ).prop( "disabled", true );
      }
     });

    $('#member_education3').click(function(){
      if ($('#member_education3').is(':checked') == true) {
        $( "#member_major3" ).prop( "disabled", false );
        $( "#member_institution3" ).prop( "disabled", false );
      }else {
        $( "#member_major3" ).prop( "disabled", true );
        $( "#member_institution3" ).prop( "disabled", true );
      }
    });

    $('#member_education4').click(function(){
      if ($('#member_education4').is(':checked') == true) {
        $( "#member_major4" ).prop( "disabled", false );
        $( "#member_institution4" ).prop( "disabled", false );
      }else {
        $( "#member_major4" ).prop( "disabled", true );
        $( "#member_institution4" ).prop( "disabled", true );
      }
    });
...
var get_profile_member = {
  ...
  data_profile_member:() => {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        // console.log(members.member_education);
        // console.log(members.member_education.มัธยมศึกษา);
       ...
       // STR += '<option value="">'+study_group_text+'</option>';
        ...
        if (data.lang == 'th') {
          $('#lang').val('th');
          var STR_SCHOOL = '<option value="">' +"------ กรุณาเลือกโรงเรียน ------"+ '</option>';
        }else {
          $('#lang').val('en');
          var STR_SCHOOL = '<option value="">' +"------ Please select a school. ------"+ '</option>';
        }

        // console.log(data.members[0].member_school);
        if (data.members[0].member_school == "" || data.members[0].member_school == undefined) {
          for (var i = 0; i < data.school.length; i++) {
            if (data.members[0].member_school == data.school[i]._id) { //เช็คไอดี member ในตาราง school ถ้ามีให้ selected
              if (data.lang == 'th') {
                $('#lang').val('th');
                STR_SCHOOL += '<option value="' + data.school[i]._id + '" selected="selected">' + data.school[i].school_name_th + '</option>';
              }else {
                $('#lang').val('en');
                STR_SCHOOL += '<option value="' + data.school[i]._id + '" selected="selected">' + data.school[i].school_name_en + '</option>';
              }
            }
            else {
              if (data.lang == 'th') {
                $('#lang').val('th');
                STR_SCHOOL += '<option value="' + data.school[i]._id + '" >' + data.school[i].school_name_th + '</option>';
              }else {
                $('#lang').val('en');
                STR_SCHOOL += '<option value="' + data.school[i]._id + '" >' + data.school[i].school_name_en + '</option>';
              }
            }

          }

          var data_school = `<div class="form-group">
                              <div class="row col-md-12">
                                <select name="school" class="form-control" style="font-size: 16px; padding-top: 4px;" >
                                `+STR_SCHOOL+`
                                </select>
                              </div>
                             </div>`;

        $('.school').html(data_school);
        }
        else {
          ...
          var data_school2 = `<div class="form-group">
                              <div class="row col-md-12">
                                <select name="school" class="form-control" style="font-size: 16px; padding-top: 4px;" disabled>
                                `+STR_SCHOOL+`
                                </select>
                              </div>
                             </div>`;

        $('.school').html(data_school2);
        }
          ...
          // setTimeout(function () {
          //   if (data.members[0].member_school) {
  ...
  confirm_updatedata : () => {
    ...
    if ($('#member_education1').is(':checked') == true) { //////////// เช็คโรงเรียนมัธยมศึกษา
      if ($('#member_institution1').val() == "") {
        ck = "n";
        if ($('#lang').val() == 'th') {
          lang_request = 'กรุณากรอกชื่อ โรงเรียน';
        }else {
          lang_request = 'Please enter school name.';
        }
      }
    }

    // else { //////////// เช็ค เช็คบล็อก มัธยมศึกษา
    //   if ($('#member_institution1').val() == "") {
    //     ck = "n";
    //     if ($('#lang').val() == 'th') {
    //       lang_request = 'กรุณาเลือกหัวข้อมัธยมศึกษา';
    //     }else {
    //       lang_request = 'Please enter school name.';
    //     }
    //   }
    ...
    if ($('#member_education2').is(':checked') == true) { //////////// ปริญญาตรี
      if ($('#member_institution2').val() == "") {
        ck = "n";
        if ($('#lang').val() == 'th') {
          lang_request = 'กรุณากรอกชื่อ มหาวิทยาลัย';
        }else {
          lang_request = 'Please enter the name of the university.';
        }
      }
      if ($('#member_major2').val() == "") {
        ck = "n";
        if ($('#lang').val() == 'th') {
          lang_request = 'กรุณากรอกชื่อ คณะ';
        }else {
          lang_request = 'Please fill out the faculty name.';
        }
      }
    }
    //
    // else { //////////// เช็ค เช็คบล็อก ปริญญาตรี
    //   ck = "n";
    //   if ($('#lang').val() == 'th') {
    //     lang_request = 'กรุณาเลือกหัวข้อปริญญาตรี';
    //   }else {
    //     lang_request = 'Please enter the name of the university.';
    //   }
    // }
    ...
    if ($('#member_education3').is(':checked') == true) { //////////// ปริญญาโท
      if ($('#member_institution3').val() == "") {
        ck = "n";
        if ($('#lang').val() == 'th') {
          lang_request = 'กรุณากรอกชื่อ มหาวิทยาลัย';
        }else {
    ...
    // else { //////////// เช็ค เช็คบล็อก ปริญญาโท
    //     ck = "n";
    //     if ($('#lang').val() == 'th') {
    //       lang_request = 'กรุณาเลือกหัวข้อปริญญาโท';
    //     }else {
    //       lang_request = 'Please enter the name of the university.';
    //     }
    // }

    if ($('#member_education4').is(':checked') == true) { //////////// ปริญญาเอก
      if ($('#member_institution4').val() == "") {
        ck = "n";
        if ($('#lang').val() == 'th') {
          lang_request = 'กรุณากรอกชื่อ มหาวิทยาลัย';
        }else {
    ...
    // else { //////////// เช็ค เช็คบล็อก ปริญญาเอก
    //
    //   if ($('#member_institution4').val() != "" || $('#member_major4').val() != "") {
    //     if ($('#lang').val() == 'th') {
    //       Swal.fire({
    //         type: 'warning',
    //         title: 'กรุณาเลือกหัวข้อปริญญาเอก',
    //         confirmButtonColor: '#28a745',
    //         confirmButtonText : close_window,
    //       });
    //       lang_request = 'กรุณากรอกชื่อ มหาวิทยาลัย';
    ...
    //       Swal.fire({
    //         type: 'warning',
    //         title: 'Please checkbox PhD topics',
    //         confirmButtonColor: '#28a745',
    //         confirmButtonText : close_window,
    //       });
    //     }
    ...
    if (ck != "y") {
     ...
     if ($('#member_education1').is(':checked') == true) {
       var member_institution = $('#member_institution').val();
     }

     if ($('#member_education2').is(':checked') == true) {
       var member_institution2 = $('#member_institution2').val();
       var member_major2 = $('#member_major2').val();
     }

     if ($('#member_education3').is(':checked') == true) {
       var member_institution3 = $('#member_institution3').val();
       var member_major3 = $('#member_major3').val();
     }

     if ($('#member_education4').is(':checked') == true) {
       var member_institution4 = $('#member_institution4').val();
       var member_major4 = $('#member_major4').val();
     }
     ...
     var data = {
       ...
       "member_institution1":member_institution,
       "member_institution2":member_institution2,
       "member_institution3":member_institution3,
       "member_institution4":member_institution4,
       "member_major2":member_major2,
       "member_major3":member_major3,
       "member_major4":member_major4,
       "member_address":$('#member_address').val(),
       "member_school":$('select[name="school"] option:selected').val(),
       ...
       $.ajax({
         ...
         success: function(data) {
             if(data.success){
                if (data.success == "เลือกโรงเรียนไม่ถูก") {
                    Swal.fire({
                    type: 'warning',
                    title: data.success,
                    confirmButtonColor: '#28a745',
