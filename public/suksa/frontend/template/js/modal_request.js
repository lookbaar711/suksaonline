...
var request = {
  make_data_request : () => {
      ...
      if ($("input[name$='day_study[]']").val() == "" && $('.study_now:checked').val() == 2) {
        ck = "n";
      }
      if ($("input[name$='start_time']").val() == "" && $('.study_now:checked').val()== 2) {
        ck = "n";
      }
      if ($( "#study_group" ).val() == "") {
        ck = "n";
      ...
      var time = "";
      var lang_reques_time = $('#lang_reques_time').val();
      var str = $("input[name$='day_study[]']").val();
      var n = str.split('/');
      var day_study = new Date(n[2]+"-"+n[1]+"-"+n[0]+" "+$("input[name$='start_time']").val()); // วันที่ ที่เลือก
      var date_select = new Date(); // วันที่ปัจุบัน

      if (day_study < date_select && $('.study_now:checked').val() == 2) {
        time = "check_time";
        ck = "n";
      }
    ...
    $.ajax({
      ...
      success: function(data) {
        if (data=="false") {
          ...
          // alert(data);
          location.href = window.location.origin + '/request/'+data;
        }
...
$(function() {
  ...
  $( ".request_subjects" ).click(function() {
    // console.log("11");
     ...
     $('textarea#details_study').val("");
     $( "#price_range" ).val("");
     $('.course_date').prop('disabled', true);
